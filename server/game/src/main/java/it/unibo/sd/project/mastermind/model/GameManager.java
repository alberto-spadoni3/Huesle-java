...
public class GameManager extends AbstractManager<Match> {
    ...
    public GameManager() {
        super.init("matches", Match.class);
        matchDB = super.database;
        pendingMatchDB = new DBManager<>(DATABASE_NAME, "pendingRequests", "requesterUsername", PendingMatchRequest.class);
        userDB = new DBManager<>(DATABASE_NAME, "users", "username", Player.class);
    }
    ...
    protected Map<MessageType, Function<String, String>> getManagementCallbacks() {
        ...
        gameCallbacks.put(MessageType.SEARCH_MATCH, searchMatch());
        gameCallbacks.put(MessageType.JOIN, joinPrivateMatch());
        return gameCallbacks;
    }

    private Function<String, String> searchMatch() {
        return message -> {
    ...
    private Function<String, String> joinPrivateMatch() {
        return message -> {
            MatchOperationResult matchOperationResult = null;
            try {
                SearchRequest request = Presentation.deserializeAs(message, SearchRequest.class);
