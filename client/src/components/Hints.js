...
const Hints = ({ isInRow }) => {
  ...
  if (attempts.length > isInRow) {
    rightPositions = attempts[isInRow].hints.rightPositions;
    rightColours = attempts[isInRow].hints.rightColours;
  }
  ...
  const generateHint = (id) => {
    ...
    return (
      <Peg key={id} isInRow={isInRow} pegID={id} hintPeg hintType={hintType} />
    );
  };
