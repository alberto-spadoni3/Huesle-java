...
const Peg = ({ pegID, isInRow, hintPeg, hintType }) => {
  ...
  useLayoutEffect(() => {
    if (hintPeg) {
      ...
      const pegsColor = attempts[isInRow].colorSequence;
      setPegColor(
        colorblindMode ? swapColors(pegsColor, true)[pegID] : pegsColor[pegID]
      );
    }
  });
  ...
  return <MyBox onClick={handleClick}>{text}</MyBox>;
};
